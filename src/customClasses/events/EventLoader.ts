...
import { BEvents, GetEventsResponse, EventRead, PlantFlowerYear } from "plants/ui/definitions/Events";
...
export default class EventLoader extends ManagedObject {
	...
	public async loadEventsForPlant(iPlantId: int) {
		...
		const oResult: GetEventsResponse = await Util.get(Util.getServiceUrl('events/' + iPlantId));
		this._cbReceivingEventsForPlant(iPlantId, oResult);
	}

	private _cbReceivingEventsForPlant(plantId: int, oData: GetEventsResponse): void {
		//insert (overwrite!) events data for current plant with data received from backend
		const aEvents: EventRead[] = oData.events;
		...
		const aFlowerHistory = <PlantFlowerYear[]>oData.flower_history;
